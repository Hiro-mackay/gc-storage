...
| フェーズ | 詳細設計ファイル | 説明 |
|---------|-----------------|------|
| Phase 0A | [infra-database.md](./infra-database.md) | PostgreSQL接続・トランザクション管理 |
| Phase 0B | [infra-redis.md](./infra-redis.md) | Redis接続・セッション・キャッシュ |
| Phase 0C | [infra-minio.md](./infra-minio.md) | MinIOストレージ・Presigned URL |
| Phase 0D | [infra-email.md](./infra-email.md) | SMTP・メールテンプレート |
| Phase 0E | [infra-api.md](./infra-api.md) | APIルーティング・ミドルウェア |
| Phase 1 | [auth-identity.md](./auth-identity.md) | 認証・JWT・OAuth・セッション |
| Phase 2A | [storage-folder.md](./storage-folder.md) | フォルダ管理（階層・閉包テーブル） |
| Phase 2B | [storage-file.md](./storage-file.md) | ファイル管理（アップロード・バージョン） |
...
📄 **詳細設計**: [infra-database.md](./infra-database.md)
...
📄 **詳細設計**: [infra-redis.md](./infra-redis.md)
...
📄 **詳細設計**: [infra-minio.md](./infra-minio.md)
...
| 0C.13 | `minio-versioning` | バージョニング有効化 | High | Draft | 0C.2 |
| 0C.14 | `minio-webhook` | Bucket Notification（Webhook受信） | High | Draft | 0C.1 |

#### MinIO 設計詳細

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    MinIO Storage Architecture（Webhook駆動）                  │
└─────────────────────────────────────────────────────────────────────────────┘

【バケット構成】
gc-storage (メインバケット、バージョニング有効)
└── {file_id}                      # ファイル本体（UUIDv4）
    └── thumbnails/{size}          # サムネイル
...
• バージョン管理: MinIOネイティブバージョニングで自動管理

【アップロードフロー（Webhook駆動）】
┌─────────┐      ┌─────────┐      ┌─────────┐      ┌─────────┐
│ Client  │─────▶│   API   │      │  MinIO  │─────▶│   API   │
│         │      │         │      │         │      │         │
│ 1.要求  │      │2.Presign│─────▶│3.Upload │      │4.Webhook│
│         │      │  URL    │      │  Direct │      │  通知   │
└─────────┘      └─────────┘      └─────────┘      └─────────┘

変更点: クライアントからの完了通知不要、MinIOが直接APIにWebhook送信
...
• 最大10,000パート
• パートサイズ: 64MB
• 初期化時に全パートのPresigned URLを一括返却
• 完了はクライアントがMinIO S3 APIを直接呼出、Webhookで検知
```

### 0D: メール送信（SMTP）

📄 **詳細設計**: [infra-email.md](./infra-email.md)
...
📄 **詳細設計**: [infra-api.md](./infra-api.md)
...
📄 **詳細設計**: [auth-identity.md](./auth-identity.md)
...
📄 **詳細設計**:
- [storage-folder.md](./storage-folder.md) - フォルダ管理（閉包テーブルによる階層構造）
- [storage-file.md](./storage-file.md) - ファイル管理（Presigned URL、バージョニング）

### 2A: フォルダ管理

📄 **詳細設計**: [storage-folder.md](./storage-folder.md)
...
📄 **詳細設計**: [storage-file.md](./storage-file.md)
...
📄 **詳細設計**: [collab-group.md](./collab-group.md)
...
📄 **詳細設計**: [authz-permission.md](./authz-permission.md)
...
📄 **詳細設計**: [share-link.md](./share-link.md)
